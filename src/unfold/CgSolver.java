 ...
 * Modified to reduce unnecessary applications of the left-hand-side
 * Hessian operator when the input vector x is zero.
 * @author Dave Hale and Simon Luo, Colorado School of Mines
 * @version 2013.11.25
 */
public class CgSolver {
  ...
  public Info solve(A a, Vec b, Vec x) {
    Vec q = b.clone();
    if (x.norm2()<100.0*Double.MIN_VALUE)
      q.zero();
    else
      a.apply(x,q); // q = Ax
    Vec r = b.clone();
